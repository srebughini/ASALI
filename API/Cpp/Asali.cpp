...
#include "shared/omega22.H"
    ...
    std::vector<double> Asali::speciesViscosity()
    {
        speciesViscosity_();
        return mu_;
    }
    std::vector<double> Asali::speciesMolarCp()
    {
        speciesCp_();
        return cpmole_;
    }
    std::vector<double> Asali::speciesMassCp()
    {
        speciesCp_();
        return cpmass_;
    }
    std::vector<double> Asali::speciesMolarEnthalpy()
    {
        speciesH_();
        return hmole_;
    }
    std::vector<double> Asali::speciesMassEnthalpy()
    {
        speciesH_();
        return hmass_;
    }
    std::vector<double> Asali::speciesMolarEntropy()
    {
        speciesS_();
        return smole_;
    }
    std::vector<double> Asali::speciesMassEntropy()
    {
        speciesS_();
        return smass_;
    }
    std::vector<double> Asali::speciesThermalConductivity()
    {
        speciesThermalConductivity_();
        return cond_;
    }
    std::vector<std::vector<double>> Asali::binaryDiffusion()
    {
        binaryDiffusion_();
        return diff_;
    }
    ...
    void Asali::speciesThermalConductivity_() //[W/m/K]
    {
        if (!cond_update_)
        {
            ...
            for (unsigned int i = 0; i < NC_; i++)
            {
                ...
                {
                    double F_T = 1. + 0.5 * std::sqrt(std::pow(pi_, 3.) * transport_.LJpotential[index_[i]] / T_) + (0.25 * std::pow(pi_, 2.) + 2.) * (transport_.LJpotential[index_[i]] / T_) + std::sqrt(std::pow(pi_ * transport_.LJpotential[index_[i]] / T_, 3.));
                    double F_298 = 1. + 0.5 * std::sqrt(std::pow(pi_, 3.) * transport_.LJpotential[index_[i]] / 298.) + (0.25 * std::pow(pi_, 2.) + 2.) * (transport_.LJpotential[index_[i]] / 298.) + std::sqrt(std::pow(pi_ * transport_.LJpotential[index_[i]] / 298., 3.));
    ...
    void Asali::speciesCp_()
    {
        if (!cp_update_)
        {
            for (unsigned int i = 0; i < NC_; i++)
            {
                if (T_ < 1000.)
                {
                    cpmole_[i] = thermo_.low[index_[i]][0] + thermo_.low[index_[i]][1] * T_ + thermo_.low[index_[i]][2] * std::pow(T_, 2.) + thermo_.low[index_[i]][3] * std::pow(T_, 3.) + thermo_.low[index_[i]][4] * std::pow(T_, 4.);
                }
                else
                {
                    cpmole_[i] = thermo_.high[index_[i]][0] + thermo_.high[index_[i]][1] * T_ + thermo_.high[index_[i]][2] * std::pow(T_, 2.) + thermo_.high[index_[i]][3] * std::pow(T_, 3.) + thermo_.high[index_[i]][4] * std::pow(T_, 4.);
                }
    ...
    void Asali::speciesH_()
    {
        if (!h_update_)
        {
            for (unsigned int i = 0; i < NC_; i++)
            {
                if (T_ < 1000.)
                {
                    hmole_[i] = thermo_.low[index_[i]][0] + thermo_.low[index_[i]][1] * T_ / 2. + thermo_.low[index_[i]][2] * std::pow(T_, 2.) / 3. + thermo_.low[index_[i]][3] * std::pow(T_, 3.) / 4. + thermo_.low[index_[i]][4] * std::pow(T_, 4.) / 5. + thermo_.low[index_[i]][5] / T_;
                }
                else
                {
                    hmole_[i] = thermo_.high[index_[i]][0] + thermo_.high[index_[i]][1] * T_ / 2. + thermo_.high[index_[i]][2] * std::pow(T_, 2.) / 3. + thermo_.high[index_[i]][3] * std::pow(T_, 3.) / 4. + thermo_.high[index_[i]][4] * std::pow(T_, 4.) / 5. + thermo_.high[index_[i]][5] / T_;
                }
    ...
    void Asali::speciesS_()
    {
        if (!s_update_)
        {
            for (unsigned int i = 0; i < NC_; i++)
            {
                if (T_ < 1000.)
                {
                    smole_[i] = thermo_.low[index_[i]][0] * std::log(T_) + thermo_.low[index_[i]][1] * T_ + thermo_.low[index_[i]][2] * std::pow(T_, 2.) / 2. + thermo_.low[index_[i]][3] * std::pow(T_, 3.) / 3. + thermo_.low[index_[i]][4] * std::pow(T_, 4.) / 4. + thermo_.low[index_[i]][6];
                }
                else
                {
                    smole_[i] = thermo_.high[index_[i]][0] * std::log(T_) + thermo_.high[index_[i]][1] * T_ + thermo_.high[index_[i]][2] * std::pow(T_, 2.) / 2. + thermo_.high[index_[i]][3] * std::pow(T_, 3.) / 3. + thermo_.high[index_[i]][4] * std::pow(T_, 4.) / 4. + thermo_.high[index_[i]][6];
                }
