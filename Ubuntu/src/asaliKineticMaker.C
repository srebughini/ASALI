...
namespace ASALI
{
    asaliKineticMaker::asaliKineticMaker()
      ...
      restart_(true)
    {
        ...
        //Reaction
        {
            ...
            backButton2_.signal_clicked().connect(sigc::mem_fun(*this,&asaliKineticMaker::species));
            reactionGrid_.attach(reactionHelpButton_,0,5,1,1);
            reactionHelpButton_.signal_clicked().connect(sigc::mem_fun(*this,&asaliKineticMaker::helpReaction));
            reactionGrid_.attach(nextButton3_,2,5,1,1);
            nextButton3_.signal_clicked().connect(sigc::mem_fun(*this,&asaliKineticMaker::equation));
        ...
        //Equation
        {
            ...
            backButton3_.signal_clicked().connect(sigc::mem_fun(*this,&asaliKineticMaker::reaction));
            equationGrid_.attach(equationHelpButton_,0,5,1,1);
            equationHelpButton_.signal_clicked().connect(sigc::mem_fun(*this,&asaliKineticMaker::helpEquation));
            equationGrid_.attach(nextButton4_,2,5,1,1);
            nextButton4_.signal_clicked().connect(sigc::mem_fun(*this,&asaliKineticMaker::recap));
        ...
        //Recap
        {
            ...
            backButton4_.signal_clicked().connect(sigc::mem_fun(*this,&asaliKineticMaker::equation));
            saveButton_.signal_clicked().connect(sigc::mem_fun(*this,&asaliKineticMaker::save));
    ...
    void asaliKineticMaker::reactionNumber()
    {
        this->remove();
        this->add(reactionNumberBox_);
    ...
    void asaliKineticMaker::reaction()
    {
    ...
    void asaliKineticMaker::equation()
    {
    ...
    bool asaliKineticMaker::checkReaction()
    {
        bool check = false;
        shet_.clear();
        shom_.clear();
    ...
    bool asaliKineticMaker::checkEquation()
    {
        bool check = true;
        Rtot_.clear();
        for (unsigned int i=0;i<NR_;i++)
                ...
                while ( expPos != std::string::npos )
                {
                    if ( all.at(expPos+3) == '(' )
                    {
                        ...
                        else
                        {
                            ...
                            for (std::size_t i=expPos+3;i<all.length();i++)
                            {
                                if ( all.at(i) == ')' )
                                {
                                    all.erase(i,1);
            ...
            //Check pow
            {
                ...
                while ( powPos != std::string::npos )
                {
                    if ( all.at(powPos+3) == '(' )
                    {
                        ...
                        for (std::size_t i=powPos+3;i<all.length();i++)
                        {
                            if ( all.at(i) == ',' )
                            {
                                comaPos = i;
                                if(!std::isdigit(all.at(i+1)))
                                {
                                    this->errorEquation(error);
                                    i     = NR_ - 1;
                                    check = false;
                        ...
                        for (std::size_t i=comaPos;i<all.length();i++)
                        {
                            if ( all.at(i) == ')' )
                            {
                                all.erase(i,1);
    ...
    void asaliKineticMaker::updateLayout()
    {
        restart_ = false;
        if ( NR_ != 0 )
        {
    ...
    bool asaliKineticMaker::readStoichiometryCoefficient(unsigned int index)
    {
    ...
    void asaliKineticMaker::replaceString(std::string& str, const std::string from, const std::string to)
    {
    ...
    int asaliKineticMaker::getSpecieIndex(std::string n, std::vector<std::string> nv)
    {
        int q = -1;
        for (unsigned int i=0;i<nv.size();i++)
        {
