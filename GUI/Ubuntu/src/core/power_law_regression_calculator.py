...
def power_law_regression_calculator(data_store) -> DataStore:
    """
    Function to estimate thermodynamic and transport properties power law regression
    Parameters
    ----------
    ...
    b, e, _, _, _ = stats.linregress(np.log(temperature_vector), np.log(rho))
    a = np.exp(e)
    ...
    b, e, _, _, _ = stats.linregress(np.log(temperature_vector), np.log(mu))
    a = np.exp(e)
    ...
    b, e, _, _, _ = stats.linregress(np.log(temperature_vector), np.log(cond))
    a = np.exp(e)
    ...
    b, e, _, _, _ = stats.linregress(np.log(temperature_vector), np.log(cp))
    a = np.exp(e)
    ...
    b, e, _, _, _ = stats.linregress(np.log(temperature_vector), np.log(h))
    a = np.exp(e)
    ...
    b, e, _, _, _ = stats.linregress(np.log(temperature_vector), np.log(s))
    a = np.exp(e)
