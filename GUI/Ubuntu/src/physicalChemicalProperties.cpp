...
namespace ASALI
{
    physicalChemicalProperties::physicalChemicalProperties(ASALI::canteraInterface        *canteraInterface,
      ...
      mwLabel_("Molecular weight")
    {
        //Input menu
        {
            this->title("ASALI: Physical and chemical properties");
            this->input();
        }
    ...
    void physicalChemicalProperties::save()
    {
        ...
        if ( filename != "" )
        {
            ...
            output << "Species\t" << "x            \ty            \tMW           \t\u03C1" << std::endl;
            for (unsigned int i=0;i<n_.size();i++)
            {
                if ( i == n_.size() - 1)
                {
                    output << n_[i] << "\t" << "            " << "\t" << "            " << "\t" << MW_[i] << "\t" << rho_[i] << std::endl;
                }
                else
                {
